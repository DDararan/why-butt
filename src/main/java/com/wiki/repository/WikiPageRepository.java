...
public interface WikiPageRepository extends JpaRepository<WikiPage, Long> {
    ...
    // 직접 쿼리 수행
    @Query("SELECT DISTINCT w FROM WikiPage w " +
           "WHERE LOWER(w.title) LIKE LOWER(CONCAT('%', :query, '%')) " +
           "OR LOWER(w.content) LIKE LOWER(CONCAT('%', :query, '%')) " +
           "ORDER BY updatedAt DESC")
    List<WikiPage> searchByQuery(@Param("query") String query);
    // List<WikiPage> findByTitleContainingIgnoreCaseOrContentContainingIgnoreCase(@Param("query") String query1, @Param("query") String query2);
    ...
    List<WikiPage> findByTitleContainingIgnoreCaseOrContentContainingIgnoreCase(String title, String content);
} 